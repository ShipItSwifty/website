...
`retry_on_failure` and `infrastructure_retry` solve different problems. Use `retry_on_failure` for iOS test-case retries after assertion failures. Use `infrastructure_retry` for whole-run failures such as simulator launch crashes, Android emulator disconnects, Flutter tool crashes, or JS worker failures. `shipit generate` asks whether to enable `infrastructure_retry` on generated test steps.
