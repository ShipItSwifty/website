...
### `firebase-app-distribution` action options

Use the `firebase-app-distribution` workflow action to upload a signed IPA, APK, or AAB to Firebase App Distribution. It accepts credentials only from an environment variable or a local service-account file; never put service-account JSON in `Shipfile.yml`.

| Option                 | Type   | Default | Description                                                              |
| ---------------------- | ------ | ------- | ------------------------------------------------------------------------ |
| `app_id`               | string | —       | **Required.** Firebase app ID, for example `1:1234567890:ios:abc123`.    |
| `artifact_path`        | string | —       | **Required.** Path to the signed `.ipa`, `.apk`, or `.aab` artifact.     |
| `groups`               | list   | `[]`    | Firebase tester group aliases. At least one group or tester is required. |
| `testers`              | list   | `[]`    | Individual tester email addresses.                                       |
| `release_notes`        | string | —       | Plain-text notes shown to testers.                                       |
| `service_account_path` | string | —       | Local service-account JSON file. Overrides environment credentials.      |
| `dry_run`              | bool   | `false` | Validate the planned distribution without uploading.                     |
| `timeout_seconds`      | int    | `300`   | Maximum time to wait for Firebase to process the upload.                 |

The action resolves credentials in this order: `service_account_path`, `FIREBASE_SERVICE_ACCOUNT_JSON` (raw JSON), then `GOOGLE_APPLICATION_CREDENTIALS` (path to a service-account JSON file). The Firebase app ID platform is checked against the artifact before upload, so an iOS app ID requires an IPA and an Android app ID requires an APK or AAB.
...
See [Firebase App Distribution](/docs/guides/firebase-app-distribution) for setup and Android examples.
...
- These are **distinct** from composite [`{{param.NAME}}`](#parameter-reference-syntax-paramname) references and from Shipfile `${ENV_VAR}` expansion. Resolution is scoped to top-level workflow steps; tokens inside `custom_actions` substeps are not resolved in this release.
...
AndroidCLI complements the Gradle/ADB pipeline and does not build release artifacts. Use `shipit android-cli` for direct commands; workflow actions require explicit opt-in:
...
| `cli.executable_path` | string | `android` | `SHIPIT_ANDROID__CLI__EXECUTABLE_PATH` | Executable name or path; bare names use `PATH`, while relative paths resolve from the Shipfile |
| `cli.sdk_path`        | string | —         | `SHIPIT_ANDROID__CLI__SDK_PATH`        | SDK path forwarded through AndroidCLI's global `--sdk` option                                  |

ShipIt requires AndroidCLI 1.0 or newer when enabled. It never downloads, initializes, updates, or installs skills automatically. Device- or environment-changing workflow operations require `allow_mutation: true`. AndroidCLI is preview software and may report basic command/option telemetry; emulator management is currently unavailable on Windows.
