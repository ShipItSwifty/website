...
For Android workflows, `build_variant` is respected when ShipItSwifty derives the Gradle task name. You can also bypass variant-based task selection entirely with an explicit `task` value.
...
| `module`               | string | `android.module` | Android only. Gradle module whose tests should run. Set `module: ""` with `task` to run a root-level aggregate Gradle task.                                                                                                   |
| `build_variant`        | string | `debug`      | Android only. Overrides `android.build_variant` for this test step when ShipItSwifty derives the Gradle task name.                                                                                                                |
| `instrumented`         | bool   | `false`      | Android only. Runs connected device or emulator tests instead of JVM unit tests.                                                                                                                                                   |
| `task`                 | string | —            | Android only. Explicit Gradle task name such as `testDebugUnitTest` or `testProdDebugUnitTest`. Overrides variant-based task selection.                                                                                          |
...
For Android aggregate tasks, ShipItSwifty now falls back to JUnit XML reports under `build/test-results/<taskName>/` when Gradle stdout does not include a test summary.
